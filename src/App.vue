<template>
  <div id="app">
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 350 350">
      ...
      <path @click="addLight(1);addSound(1)" class="btn btn--green btn--1" d="M40.205 167.527C43.96 98.94 98.913 43.987 167.5 40.232v75.26c-27.14 3.384-48.65 24.895-52.036 52.035h-75.26z"/>
      <path @click="addLight(2);addSound(2)" class="btn btn--red btn--2" d="M182.5 40.232c68.587 3.756 123.54 58.708 127.295 127.295h-75.26c-3.384-27.14-24.895-48.65-52.035-52.035v-75.26z"/>
      <path @click="addLight(3);addSound(3)" class="btn btn--blue btn--3" d="M309.795 182.527c-3.755 68.588-58.708 123.54-127.295 127.296v-75.26c27.14-3.384 48.65-24.895 52.036-52.036h75.26z"/>
      <path @click="addLight(4);addSound(4)" class="btn btn--yellow btn--4" d="M167.5 309.795C98.913 306.04 43.96 251.087 40.205 182.5h75.26c3.384 27.14 24.895 48.65 52.035 52.036v75.26z"/>

      <g class="counter">
        <circle class="center" cx="175" cy="175.027" r="45"/>
        <g class="counter--pieces counter--pieces__13">
      ...
      <g class="btn btn--stop" @click="clicked('stop')">
      ...
      <g class="btn btn--strict" @click="clicked('strict')">
        <path class="center--semi" d="M217.072 191.027c-6.45 16.954-22.854 29-42.072 29-19.218 0-35.62-12.046-42.072-29h84.144z"/>
        <g class="strict--text">
...
export default {
  ...
  methods: {
    ...
    startGame(){
      ...
      do {
        var lastItem = sampleArray.slice(-1);
        console.log(lastItem);
        var secondLastItem = sampleArray.slice(-2,-1);
        console.log(secondLastItem);
        currentSample = _.random(1,4);
        if (lastItem !== secondLastItem || lastItem !== currentSample) {
    ...
    addSound(num){
      synth.triggerAttackRelease(this.sounds[num-1], '16n');
    },
    errorMsg(){
      var errorSeq = new Tone.Sequence((time, note) => {
        synth.triggerAttackRelease(this.sounds[note], '32n', time);
        this.addLight(note+1);
      }, [0,1,2,3],'32n').start(0);
      errorSeq.loop = 1;
